...
// kLongRangeByteCountMask is currently unused as the byte count are always 32 bits long
// and thus we use kByteCountMask for them, but we keep it here for future use if we want
// to support 64 bits byte counts in the long range format
...
Int_t TBufferFile::ReadArray(Bool_t *&b)
{
   ...
   *this >> n;
   ...
   if (n <= 0 || static_cast<Size_t>(n) > fBufSize) return 0;
...
Int_t TBufferFile::ReadArray(Char_t *&c)
{
   ...
   *this >> n;
   ...
   if (l <= 0 || static_cast<Size_t>(l) > fBufSize) return 0;
...
Int_t TBufferFile::ReadArrayFloat16(Float_t *&f, TStreamerElement *ele)
{
   ...
   *this >> n;
   ...
   if (n <= 0 || static_cast<Size_t>(3*n) > fBufSize) return 0;
...
Int_t TBufferFile::ReadStaticArray(Float_t *f)
{
   ...
   *this >> n;
   ...
   if (n <= 0 || static_cast<Size_t>(l) > fBufSize) return 0;
...
{
   if (n <= 0 || static_cast<Size_t>(n) > fBufSize) return;
...
void TBufferFile::ReadFastArrayString(Char_t *c, Int_t n)
{
   ...
   *this >> lenchar;
   ...
   if (len) {
      if (len <= 0 || static_cast<Size_t>(len) > fBufSize) return;
      Int_t blen = len;
      if (len >= n) len = n-1;
...
{
   Int_t l = sizeof(Short_t)*n;
   if (n <= 0 || static_cast<Size_t>(l) > fBufSize) return;
...
{
   Int_t l = sizeof(Int_t)*n;
   if (l <= 0 || static_cast<Size_t>(l) > fBufSize) return;
...
{
   if (n <= 0 || static_cast<Size_t>(3*n) > fBufSize) return;
